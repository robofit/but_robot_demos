...
int start_server(int port, char *filename);
void open_kinect ();
...
int start_server(int port, char *filename)
{
    ...
    if (!filename) {
        INFO ("starting sending captured data\n");
        //read ();
    } else {
        INFO ("starting sending data from file\n");
        send_test (filename);
    }
    INFO ("disconnected\n");
    close (child_socket);
